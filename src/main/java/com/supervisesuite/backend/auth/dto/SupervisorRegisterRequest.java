...
import com.supervisesuite.backend.common.validation.SliitEmail;
...
/**
 * Inbound payload for supervisor registration.
 *
...
public class SupervisorRegisterRequest {
    ...
    /** Supervisor's given name. Max 100 characters, must not be blank. */
    @NotBlank(message = "First name is required.")
    @Size(max = 100, message = "First name must not exceed 100 characters.")
    private String firstName;

    /** Supervisor's family name. Max 100 characters, must not be blank. */
    @NotBlank(message = "Last name is required.")
    @Size(max = 100, message = "Last name must not exceed 100 characters.")
    private String lastName;

    /** Unique institutional email address used as the login identifier. */
