...
/**
 * Pure domain calculator for assembling sprint progress statistics.
 */
@Component
class JiraSprintProgressCalculator {

    private final JiraHealthClassifier classifier;

    JiraSprintProgressCalculator(JiraHealthClassifier classifier) {
        this.classifier = classifier;
    }

    public JiraSprintProgressDto calculate(List<ProjectJiraIssue> issues) {
    ...
    private void accumulateVelocity(ProjectJiraIssue issue, Map<Instant, VelocityAccumulator> velocityByWeek) {
        ...
        if (issue.getResolutionDate() != null) {
            ...
            if (issue.getJiraCreatedAt() != null && !issue.getResolutionDate().isBefore(issue.getJiraCreatedAt())) {
                double cycleDays = (issue.getResolutionDate().toEpochMilli() - issue.getJiraCreatedAt().toEpochMilli()) / 86_400_000.0;
    ...
    private boolean isBacklogGrowing(List<JiraSprintProgressDto.VelocityWeek> velocityWeeks) {
        int consecutive = 0;
        int requiredConsecutiveWeeks = 2;
    ...
    private static final class SprintAccumulator {
        ...
        private JiraSprintProgressDto.SprintSummary toSummary() {
            ...
            return new JiraSprintProgressDto.SprintSummary(
                    sprintId, sprintName, sprintState, startDate, endDate, sprintStartIssueCount,
                    completionPercent, issuesDone, issuesTotal, sprintPointsDone, sprintPointsTotal, storyPointsAvailable);
        }
    }
    ...
    private static final class VelocityAccumulator {
        ...
        private Double averageCycleDays() {
            if (resolvedWithCycleData == 0) return null;
